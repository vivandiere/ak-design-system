...
const CompactDestinationFilter = () => {
  ...
  return (
    <div className="w-full bg-white rounded-lg border border-onyx-20">
      {/* Filter List */}
      <div className="overflow-y-auto">
        {Object.entries(destinations).map(([country, data]) => (
          <div key={country} className="border-b border-onyx-20 last:border-b-0">
            {/* Country Header */}
            <div 
              className="flex items-center justify-between p-3 hover:bg-linen-50 cursor-pointer select-none transition-colors"
              ...
              <div className="flex items-center space-x-3 flex-1">
                ...
                >
                  ...
                  {isCountryPartiallySelected(country) && !isCountrySelected(country) && (
                    <div className="w-1.5 h-1.5 bg-burnt-sienna-500 rounded"></div>
                  )}
                </button>
                <span className="body-base text-onyx">{country}</span>
                <span className="body-small text-onyx-60">({data.regions.length})</span>
              </div>
              {expandedCountries[country] ? (
                <ChevronDown className="w-4 h-4 text-onyx-40" />
              ) : (
                <ChevronRight className="w-4 h-4 text-onyx-40" />
              )}
            </div>
            ...
            {expandedCountries[country] && (
              <div className="bg-linen-25">
                {data.regions.map((region) => (
                  <label 
                    key={region}
                    className="flex items-center space-x-3 p-2 pl-8 hover:bg-linen-50 cursor-pointer transition-colors"
                  >
                    <input
                      type="checkbox"
                      checked={selectedDestinations.has(region)}
                      onChange={() => toggleDestination(region)}
                      className="w-3.5 h-3.5 text-burnt-sienna-500 border-onyx-20 rounded focus:ring-burnt-sienna-500 focus:ring-2"
                    />
                    <span className="body-small text-onyx-80 flex-1">{region}</span>
                  </label>
                ))}
              </div>
            )}
...
export default CompactDestinationFilter;
